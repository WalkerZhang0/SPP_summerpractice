...
bool FileHandle::ReadOHead() {
    if (!filer_.is_open()) { return false; }
    string line;
    while (getline(filer_, line)) {
        if (line.length() >= 73 && line.substr(60, 13) == "END OF HEADER")    return true;
...
bool FileHandle::ReadOneBody(OEpoch& oepoch) {
    ...
    else { return false; } // û�л�����ݣ���ʾ�Ѿ������ļ���
    for (int i = 0; i < oepoch.numsat; i++)
    {
        // if(i == oepoch.numsat){break;}
        // else{
        getline(filer_, line);
        if (line[0] == 'G'&& line.length()>=67) {
            temps.clear();
            temp = line.substr(0, 3);
            Erase(temp);
            gnss_.prn = temp;
            temp = line.substr(4, 14);
            temps.push_back(temp);
            temp = line.substr(19, 16);
            temps.push_back(temp);
            temp = line.substr(36, 14);
            temps.push_back(temp);
            temp = line.substr(52, 16);
            temps.push_back(temp);
            temp = line.substr(68, 16);
            temps.push_back(temp);
            temp = line.substr(84, 16);
            temps.push_back(temp);
    ...
    if (count >= 4) oepoch.isvaild = true;
    else oepoch.isvaild = false;
    return true;
}
...
bool FileHandle::ReadNHead() {
    string line, temp;
    vector<double> data;
    if (!filen_.is_open())   return false;
